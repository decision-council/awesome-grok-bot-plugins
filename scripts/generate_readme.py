#!/usr/bin/env python3
"""Emit README.md from data/plugins.json — the single source of truth.
...
Render switches live in data/plugins.json meta.render, never CLI flags, so the
flagless CI --check is deterministic against versioned input. Content is emitted
verbatim — no punctuation or whitespace normalization. Provenance markers (C)
are stored in the data even when render-suppressed: awesome-lint 2.3.0 rejects
a bold-code marker before a description that does not start with an uppercase
Latin letter.
"""
...
OPTIONAL_ENTRY_KEYS = {"subcategory", "slug"}
HELD_KEYS = {"name", "catalog_description", "why_held"}
HELD_HEADER = ("Plugin", "Catalog description", "Why held")
EMPTY_HELD = (
    "None this capture. Every listed plugin has a verified Cursor Marketplace "
    "URL from the Grok Bot in-app catalog plus the public marketplace listing."
)
...
def render_sections(data, by_cat):
    ...
    for cat in data["categories"]:
        ...
        if render["category_counts"]:
            n = len(entries)
            lines += [f"{n:,} plugin{'s' if n != 1 else ''}.", ""]
...
def suppressed_markers(plugins):
    return [e["name"] for e in plugins if e.get("marker") and not marker_renderable(e)]
...
def validate(data, template):
    ...
    if not isinstance(data, dict) or set(data) != {
        ...
        return ["top-level keys must be exactly meta/categories/plugins/held"]
    ...
    if not isinstance(data["plugins"], list) or not data["plugins"]:
        add("plugins must be a non-empty list")
    for e in data["plugins"] if isinstance(data["plugins"], list) else []:
        if not isinstance(e, dict):
            add(f"plugin is not an object: {e!r}")
            continue
        label = (
            e.get("name")
        ...
        if e.get("marker") not in ("C", None):
            add(f"{label}: marker must be \"C\" or null, got {e.get('marker')!r}")
        cat = e.get("category")
    ...
    for low, group in _grouped_dict(names):
        if len(group) > 1:
            add(f"duplicate plugin name (case-insensitive): {group}")
    ...
    if isinstance(data["plugins"], list):
        ...
        for c in cats:
            if not used[c]:
                add(f"category has no plugins: {c}")
        if subheadings_on:
            for c in cats:
                entries = [
                    e
    ...
    held_names = {}
    if not isinstance(data["held"], list):
        add("held must be a list (empty is allowed when every catalog entry verified)")
...
def main():
    ...
    if errors:
        ...
        sys.exit(f"{len(errors)} validation error(s) in data/plugins.json")
    ...
    summary = (
        f"{len(data['plugins']):,} listed + {len(data['held'])} held | "
        f"{len(data['categories'])} categories | {len(out_bytes):,} bytes"
    )
    if "--check" in sys.argv:
        if not README.exists() or README.read_bytes() != out_bytes:
            sys.exit(
                "OUT OF DATE — edit data/plugins.json, then run: python3 scripts/generate_readme.py"
            )
        print(f"README.md matches data/plugins.json — {summary}")
    else:
        README.write_bytes(out_bytes)
        print(f"wrote README.md — {summary}")
    sup = suppressed_markers(data["plugins"])
    if sup:
        print(
